...
def brute_force(points):
    ...
    return tours , best_tour, min_distance , best_tour_i
...
if not points:
    ...
    tours , best_tour, min_distance , best_tour_i = brute_force(points)
    ...
    # ציור המסלול
    plt.figure(figsize=(6,6))
    plt.scatter(*zip(*best_tour), color='red', label='Points')  # ציור הנקודות
    plt.plot(tour_x, tour_y, color='blue', marker='o', label='best_tour Path')  # ציור המסלול כולל החזרה לנקודת המוצא
    
    # הוספת כותרת עם אורך המסלול
    plt.title(f'best_tour {best_tour_i+1} - Distance: {min_distance:.2f}')
 ...
 ################################
    ...
    Enter = input("Press Enter to start save all tours")
